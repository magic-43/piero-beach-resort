...
export default async function AdminHomePage() {
  ...
  const currentDiscount = Number(settingsData?.global_discount_percentage) || 0;

  // Group by Date for the new transaction list
        ...
        <div className="mx-4 bg-white text-resort-cocoa rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative mt-2">
          ...
          <div className="p-6 relative z-10 flex flex-col">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1">Total Picking</p>
              ...
              {/* Metrics List inside the card on the right */}
              <div className="flex flex-col space-y-3 text-right mt-1">
                ...
                <div>
                  <p className="text-[9px] text-slate-400 uppercase tracking-widest font-bold">Discount</p>
                  <p className="font-semibold text-resort-cocoa text-sm">{currentDiscount}%</p>
                </div>
                <div>
                    ...
                    <div className="divide-y divide-gray-50">
                      {groupedSubmissions[dateStr].map((sub) => {
                        ...
                        return (
                          <Link 
                            key={sub.id} 
                            href={`/admin/payments/${booking?.reference}`}
                            className="flex items-center group cursor-pointer p-4 hover:bg-gray-50/50 transition-colors"
                          >
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 mr-4 shadow-sm border ${
                              isGCash ? "bg-[#007DFE]/10 border-[#007DFE]/20" : "bg-emerald-50 border-emerald-100"
                            }`}>
                              {isGCash ? (
                                <ArrowDownLeft className="w-5 h-5 text-[#007DFE]" />
                              ) : (
                            ...
                            <div className="flex-1 min-w-0 pr-3">
                              ...
                              <p className="text-[12px] text-slate-500 truncate flex items-center mt-0.5">
                                {isGCash ? "GCash" : "Bank"}
                            ...
                            <div className="text-right shrink-0">
                              <p className="text-[14px] font-bold text-emerald-600 tracking-tight">
                                +₱{Number(sub.amount_claimed).toLocaleString()}
                              </p>
      ...
      <div className="hidden md:flex flex-col space-y-6 font-sans pb-12 text-resort-cocoa">
        <div className="grid grid-cols-4 gap-5">
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100/50">
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2">Total Balance</p>
          ...
          <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100/50">
            <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2">Discount</p>
            <p className="font-serif text-3xl">{currentDiscount}%</p>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100/50 overflow-hidden mt-2">
                  ...
                  return (
                    <tr key={sub.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="py-4 px-6">
                        <Link href={`/admin/payments/${booking?.reference}`} className="font-medium hover:text-[#c4a47c] transition-colors">
                          {booking?.guest_name}
                        </Link>
                      </td>
                      <td className="py-4 px-6">
                        <span className="text-gray-400 font-medium">{booking?.reference}</span>
                      </td>
                      <td className="py-4 px-6">
                        <span className="text-gray-500 font-medium">{roomName}</span>
                      </td>
                      <td className="py-4 px-6">
                        <span className="text-gray-500 font-medium">{format(new Date(sub.created_at), "MMM d, yyyy")}</span>
                      </td>
                      <td className="py-4 px-6">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest ${
                          isGCash ? "bg-blue-50 text-blue-700" : "bg-emerald-50 text-emerald-700"
                        }`}>
                          {isGCash ? "GCash" : "Bank"}
                        </span>
                      </td>
