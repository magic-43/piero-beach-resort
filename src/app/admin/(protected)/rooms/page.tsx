      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {rooms?.map((room) => {
          ...
          return (
            <div
              key={room.id}
              className="bg-resort-white rounded-3xl overflow-hidden border border-resort-cocoa/10 shadow-sm flex flex-col sm:flex-row h-auto group hover:shadow-md transition-shadow"
            >
              <div className="relative w-full sm:w-2/5 h-56 sm:h-full min-h-[250px] bg-resort-sand shrink-0 overflow-hidden">
                ...
                <div className="absolute bottom-3 right-3 text-white px-2 py-1 flex flex-col items-end text-right drop-shadow-md">
                  <span className="line-through opacity-70 text-[10px] font-bold tracking-widest">
                    ₱{room.regular_rate.toLocaleString()}
                  </span>
                  <span className="font-serif text-xl">
                    ₱{room.discounted_rate.toLocaleString()}{" "}
              ...
              <div className="p-5 flex flex-col justify-between flex-1">
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-y-2 text-sm text-resort-cocoa/70">
                    <div className="flex items-center">
                      ...
                      <span>
                        <span className="font-medium text-resort-cocoa/90">{room.standard_guests} std. adults</span> +{" "}
                        {room.max_extra_guests} max extra
                      </span>
                    </div>
                    <div className="flex items-center">
                      <Coffee className="w-4 h-4 mr-3 opacity-50 shrink-0" />
                      <span>
                        Breakfast for <span className="font-medium text-resort-cocoa/90">{room.breakfast_guests}</span> included
                      </span>
                    </div>
                    <div className="flex items-center">
                ...
                <div className="mt-6 pt-4 border-t border-resort-cocoa/10 flex items-center gap-3">
                  ...
                  <Link
                    ...
                    className="flex-1 text-center bg-resort-cocoa/5 text-resort-cocoa py-2 rounded-lg text-xs font-bold uppercase tracking-widest border border-resort-cocoa/10 hover:bg-resort-cocoa hover:text-white transition-colors"
                  >
                    Edit Room
                  </Link>
                </div>
